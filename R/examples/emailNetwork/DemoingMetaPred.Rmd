...
plot_res(merge_res, title = "Accuracy with Small Departments Merged")
```


# Applying to Carnegie Melon FB Data

```{r}
data_dir =  "~/Documents/GitHub/latentChannelPaper/data/"
edge_fn = paste0(data_dir, "FBCarnegieEdges.csv")
meta_fn = paste0(data_dir, "FBCarnegieMeta.csv")

carn_edges = read.csv(edge_fn)
carn_meta = read.csv(meta_fn)

meta_year = data.frame(year = carn_meta$V6)
year_res1 = mult_acc(carn_edges, meta_year, 
                    K = c(10, 50), MC = 4) 
plot_res(year_res1)

year_res2 = mult_acc(carn_edges, meta_year, 
                    K = c(100, 200), MC = 4) 
