#' @import pheatmap grid

cols = c("darkblue", "orange", "red")
colFxn = colorRampPalette(cols)
#' @export 
#' @description 
#' Default color gradient used by heatmapLCN
#' @example 
#' # Creation of default_LCN_cgrad:
#' cols = c("darkblue", "orange", "red")
#' colFxn = colorRampPalette(cols)
#' default_LCN_cgrad = colFxn(256)
default_LCN_cgrad = colFxn(256)
...
heatmapLCN = function(lcn_mod, 
  ...
  grid::grid.text(xlab, y=-0.07, gp=grid::gpar(fontsize=16))
  grid::grid.text(ylab, x=-0.07, rot=90, gp=grid::gpar(fontsize=16))
